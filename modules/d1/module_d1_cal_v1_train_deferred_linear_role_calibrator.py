...
# -*- coding: utf-8 -*-
"""
Train D1-CAL V1 deferred linear role calibrator from a traceable dataset.
"""
...
def load_rows(dataset_root: Path, sample_ids: Sequence[str]) -> List[Dict[str, str]]:
    ...
    for sid in sample_ids:
        rows.extend(read_csv(dataset_root / "samples" / sid / "tables" / "linearity_features.csv"))
    return rows
...
def matrix(rows: Sequence[Dict[str, str]], mean: Dict[str, float] | None = None, std: Dict[str, float] | None = None) -> Tuple[np.ndarray, np.ndarray, Dict[str, float], Dict[str, float]]:
    ...
    y = np.array([TARGETS.index(row["role_label"]) for row in rows], dtype=np.int64)
    if mean is None:
...
def class_weights(y: np.ndarray) -> np.ndarray:
    ...
    return total / (len(TARGETS) * np.maximum(counts, 1.0))
...
def train(x: np.ndarray, y: np.ndarray, cfg: TrainConfig) -> Tuple[np.ndarray, List[Dict[str, float]], Dict[str, float]]:
    ...
    return weights, history, {TARGETS[idx]: float(cls_weights[idx]) for idx in range(len(TARGETS))}
...
def coefficient_rows(weights: np.ndarray) -> List[Dict[str, Any]]:
    ...
    for idx, name in enumerate(FEATURE_NAMES + ["bias"]):
...
def run(dataset_root: Path, model_out: Path, cfg: TrainConfig) -> Dict[str, Any]:
    ...
    write_json(
        model_out / "model_config.json",
        {
            ...
            "role_code": {role: idx + 1 for idx, role in enumerate(TARGETS)},
