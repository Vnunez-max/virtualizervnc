...
EXPECTED_MODULES = [
    ...
    "c1/module_c1_cal_v1_residual_hypothesis_dataset.py",
    "c1/module_c1_cal_v1_train_residual_hypothesis_calibrator.py",
    "c1/module_c1_cal_v1_apply_residual_hypothesis_calibrator.py",
    ...
    "d1/module_d1_cal_v1_deferred_linear_role_dataset.py",
    "d1/module_d1_cal_v1_train_deferred_linear_role_calibrator.py",
    "d1/module_d1_cal_v1_apply_deferred_linear_role_calibrator.py",
...
def main() -> int:
    ...
    model_dirs = [
        ("G1.0-CAL V1", ROOT / "models" / "g1_0_cal_v1_deferred_family"),
        ("C1-CAL V1", ROOT / "models" / "c1_cal_v1_residual_hypothesis"),
        ("D1-CAL V1", ROOT / "models" / "d1_cal_v1_deferred_linear_role"),
    ]
    for label, model_dir in model_dirs:
        ...
        if missing_model_files:
            return fail(f"faltan archivos de modelo {label}: {missing_model_files}")
        ...
        if not config.get("feature_names") or not config.get("targets"):
            return fail(f"{label} model_config.json no contiene feature_names/targets")
        if not scaler.get("mean") or not scaler.get("std"):
            return fail(f"{label} feature_scaler.json no contiene mean/std")
        if not coeff_rows:
            return fail(f"{label} coefficients.csv esta vacio")
    print("PASS modelos G1.0-CAL, C1-CAL y D1-CAL presentes y legibles")
